  ...
  return (
    <Link
      ...
      className="grid grid-cols-[1fr_160px_120px_100px] gap-4 items-center
                 px-5 py-3.5 border-b border-gray-100 last:border-0
                 hover:bg-gray-50/70 transition-colors duration-100 group"
    >
      {/* Nome + especialidade */}
      ...
      <div className="text-xs text-gray-500">
        <div className="font-medium text-gray-700">{formatDate(p.plano_fim, 'dd MMM yy')}</div>
        {diasFim <= 30 && diasFim >= 0 ? (
          ...
          <div className="text-gray-400 mt-0.5">{diasFim}d restantes</div>
        )}
      </div>
      ...
      <div>
        ...
        <div className="text-[11px] text-gray-500">{scoreLabel(p.score)}</div>
      </div>
